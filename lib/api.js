...
async function getEvent(id) {
  ...
  // Detail có thể trả wrap {success,data} → fetchJSON đã unwrap
  // Nếu không có thì thử fallback search trong fixtures hôm nay
  if (data) return normFixture(data);
...
async function getStandings(leagueKey) {
  ...
  const rows = Array.isArray(raw) ? raw : (raw && raw.standings ? raw.standings : []);
  return {
    league: lg,
    rows: rows.map(r => ({
...
async function getLeagueFixtures(leagueKey) {
  ...
  return (Array.isArray(raw) ? raw : []).map(normFixture).filter(Boolean);
}
