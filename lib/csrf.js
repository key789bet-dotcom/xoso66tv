...
// Whitelist paths — KHÔNG check CSRF (cho webhooks, health check, auth endpoint)
const SKIP_PATHS = [
  /^\/api\/health$/,
  /^\/api\/ping/,
  /^\/api\/webhook\//,           // webhook từ partner xoso66
  /^\/api\/srs\//,                // SRS callback (publish/play)
  // 🔐 Auth endpoints — KHÔNG cần CSRF vì attacker không có credentials
  //    (CSRF chỉ có nghĩa cho user đã authenticated)
  /^\/api\/auth\/login$/,
  /^\/api\/auth\/register$/,
  /^\/api\/auth\/forgot/,
  /^\/api\/auth\/reset/,
...
function setCookie(res, token) {
  ...
  // ⚠️ Dùng res.append('Set-Cookie', ...) thay setHeader để KHÔNG override
  //    cookie từ middleware khác (analytics, jwt, ...)
  const cookieStr = COOKIE_NAME + '=' + encodeURIComponent(token) +
                    '; Path=/; SameSite=Lax; Max-Age=' + COOKIE_TTL_S +
                    (process.env.NODE_ENV === 'production' ? '; Secure' : '');
  if (typeof res.append === 'function') {
    res.append('Set-Cookie', cookieStr);
  } else {
    // Fallback: merge với existing
