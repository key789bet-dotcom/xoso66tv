...
async function tick() {
  ...
  // Tìm schedule approved, hết hạn (endTime + GRACE < now), chưa được mark ended
  const expired = schedules.filter(s =>
    s.status === 'approved' &&
    typeof s.endTime === 'number' &&
    (s.endTime + GRACE_MS) < now
  );
  ...
  for (const sch of expired) {
    ...
    // 2. Set liveNow=false + clear startedAt
    if (user) {
      user.liveNow = false;
      delete user.liveStartedAt;
      // 3. Rotate stream key → key cũ vô hiệu
      const prefix = user.id || ('u_' + (user.username || sch.username));
      user.streamKey = _genStreamKey(prefix);
      mutated = true;
    }

    // 4. Mark schedule ended
    sch.status = 'ended';
    sch.endedAt = now;
    sch.endReason = 'auto_end_by_schedule';
    sch.kicked = kicked;
    mutated = true;
