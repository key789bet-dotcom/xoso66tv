    ...
    if (result.blocked) {
      ...
      if (fail2banTag) {
        const realIp = req.headers['cf-connecting-ip'] ||
                       ...
                       req.ip || 'unknown';
        const line = '[FAIL2BAN] ' + new Date().toISOString() + ' ' + fail2banTag +
                     ' blocked ip=' + realIp + ' path=' + req.path;
        // Log to PM2 console
        console.log(line);
        // 🛡️ ALSO write to dedicated audit file cho fail2ban watch
        //    (tránh phụ thuộc PM2 cluster log buffering)
        try {
          require('fs').appendFile(
            process.env.SECURITY_LOG || '/var/log/xoso66tv-security.log',
            line + '\n',
            function(){}
          );
        } catch (_) {}
...
// Admin login: 10 lần / 15 phút (vẫn nghiêm ngặt vs public, vừa đủ cho admin debug)
const adminLoginLimiter = createLimiter({
  max: 10, windowMs: 15*60*1000,
  message: 'Quá nhiều lần thử admin. Đợi 15 phút.',
  keyGenerator: _keyByIpAndField('username'),
