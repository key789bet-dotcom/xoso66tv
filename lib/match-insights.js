...
/**
 * Stats thethaoviet — schema THỰC: FLAT array
 *   [{team_id, stat_type, stat_value}, ...]
 *
 * Các stat_type đã thấy:
 *   Shots on Goal, Shots off Goal, Total Shots, Blocked Shots,
 *   Shots insidebox, Shots outsidebox, Fouls, Corner Kicks, Offsides,
 *   Ball Possession ("48%"), Yellow Cards, Red Cards, Goalkeeper Saves,
...
function _parseStatsThethao(rawStats, homeId, awayId) {
  ...
  // Fallback shape A (nested api-sports.io) — vẫn support nếu API đổi
  if (rawStats[0] && rawStats[0].statistics && Array.isArray(rawStats[0].statistics)) {
    return _parseStats(rawStats, homeId, awayId);
  }
  // Schema FLAT (thực tế thethaoviet)
  // Group theo team_id → { 26: {'Shots on Goal':'6', ...}, 1532: {...} }
  const byTeam = {};
  for (const row of rawStats) {
    if (!row || row.team_id == null || !row.stat_type) continue;
    const tid = String(row.team_id);
    if (!byTeam[tid]) byTeam[tid] = {};
    byTeam[tid][row.stat_type] = row.stat_value;
  }
  const h = byTeam[String(homeId)] || {};
  const a = byTeam[String(awayId)] || {};
  if (Object.keys(h).length === 0 && Object.keys(a).length === 0) return null;

  // Parser numeric — giữ % nếu có
  function n(v) { if (v == null) return 0; const s = String(v).replace('%',''); const x = parseFloat(s); return isNaN(x) ? 0 : x; }
  function pick(o, keys) { for (const k of keys) if (o[k] != null) return o[k]; return null; }

  return {
    possession: { home: n(pick(h, ['Ball Possession'])),       away: n(pick(a, ['Ball Possession'])) },
    shots:      { home: n(pick(h, ['Total Shots'])),           away: n(pick(a, ['Total Shots'])) },
    shotsOn:    { home: n(pick(h, ['Shots on Goal'])),         away: n(pick(a, ['Shots on Goal'])) },
    shotsOff:   { home: n(pick(h, ['Shots off Goal'])),        away: n(pick(a, ['Shots off Goal'])) },
    shotsBlocked:{home: n(pick(h, ['Blocked Shots'])),         away: n(pick(a, ['Blocked Shots'])) },
    shotsInBox: { home: n(pick(h, ['Shots insidebox'])),       away: n(pick(a, ['Shots insidebox'])) },
    shotsOutBox:{ home: n(pick(h, ['Shots outsidebox'])),      away: n(pick(a, ['Shots outsidebox'])) },
    corners:    { home: n(pick(h, ['Corner Kicks'])),          away: n(pick(a, ['Corner Kicks'])) },
    fouls:      { home: n(pick(h, ['Fouls'])),                 away: n(pick(a, ['Fouls'])) },
    offsides:   { home: n(pick(h, ['Offsides'])),              away: n(pick(a, ['Offsides'])) },
    cards: {
      home: { yellow: n(pick(h, ['Yellow Cards'])), red: n(pick(h, ['Red Cards'])) },
      away: { yellow: n(pick(a, ['Yellow Cards'])), red: n(pick(a, ['Red Cards'])) }
    },
    saves:      { home: n(pick(h, ['Goalkeeper Saves'])),      away: n(pick(a, ['Goalkeeper Saves'])) },
    passes:     { home: n(pick(h, ['Total passes'])),          away: n(pick(a, ['Total passes'])) },
    passesAccurate: { home: n(pick(h, ['Passes accurate'])),   away: n(pick(a, ['Passes accurate'])) },
    passesPct:  { home: n(pick(h, ['Passes %'])),              away: n(pick(a, ['Passes %'])) },
    xG:         { home: n(pick(h, ['expected_goals'])),        away: n(pick(a, ['expected_goals'])) },
    goalsPrevented: { home: n(pick(h, ['goals_prevented'])),   away: n(pick(a, ['goals_prevented'])) }
  };
}

/**
 * Events thethaoviet — schema THỰC: FLAT array
 *   [{time_elapsed, time_extra, team_id, player_id, player_name, assist_id, assist_name, type, detail, comments}]
 *
 * type ∈ {Goal, Subst, Card, Var}
 * detail:
 *   Goal:  'Normal Goal' | 'Own Goal' | 'Penalty' | 'Missed Penalty'
 *   Card:  'Yellow Card' | 'Red Card' | 'Second Yellow Card'
 *   Subst: 'Substitution N' (player_name = OUT, assist_name = IN)
...
function _parseEventsThethao(rawEvents, homeId) {
  ...
  // Fallback shape api-sports.io nested
  if (rawEvents[0] && rawEvents[0].time && rawEvents[0].team) {
    return _parseEvents(rawEvents, homeId);
  }
  return rawEvents.map(ev => {
    const minute = Number(ev.time_elapsed || 0);
    const extra  = ev.time_extra ? Number(ev.time_extra) : 0;
    ...
    const dlc = rawDetail.toLowerCase();
    let type = 'other';
    let subType = '';
    if (rawType === 'Goal') {
      type = 'goal';
      if (dlc.includes('own goal'))         subType = 'own';
      else if (dlc.includes('missed penalty')) subType = 'missed_pen';
      else if (dlc.includes('penalty'))     subType = 'penalty';
      else                                  subType = 'normal';
    } else if (rawType === 'Card') {
      if (dlc.includes('red') || dlc.includes('second yellow')) type = 'red';
      else                                                       type = 'yellow';
