...
async function fetchRawOdds(fixtureId, opts) {
  ...
  // Fix bug: BASE = 'https://api.thethaoviet.vip/api' → DETAIL = BASE + '/p'
  const DETAIL = BASE + '/p';
  // Ưu tiên endpoints upcoming/pre-match
  const candidates = [
    // ✅ CONFIRMED working — /detail trả full fixture (cần check có odds bên trong)
    DETAIL + '/fixtures/' + fixtureId + '/detail',
    // Variations có thể có odds
    DETAIL + '/fixtures/' + fixtureId + '/odds',
    DETAIL + '/fixtures/' + fixtureId + '/detail?odds=1',
    DETAIL + '/fixtures/' + fixtureId + '/detail?include=odds',
    DETAIL + '/fixtures/' + fixtureId + '/h2h',
    DETAIL + '/fixtures/' + fixtureId + '/statistics',
    DETAIL + '/fixtures/' + fixtureId + '/lineups',
    DETAIL + '/fixtures/' + fixtureId + '/predictions',
    DETAIL + '/odds/' + fixtureId,
    DETAIL + '/odds?fixture=' + fixtureId,
    DETAIL + '/fixtures/' + fixtureId,
    DETAIL + '/match/' + fixtureId,
    DETAIL + '/match/' + fixtureId + '/odds',
    DETAIL + '/match-odds/' + fixtureId,
    DETAIL + '/football-match/' + fixtureId,
    DETAIL + '/football-match/du-lieu/' + fixtureId,
    // BASE candidates
...
function parseOdds(raw) {
  ...
  // ═══ Format 0: thethaoviet.vip /api/p/fixtures/{id}/detail ═══
  // Path: data.odds.match_winner[] / asian_handicap[] / goals_over_under[]
  // Mỗi item: { bet_id, bet_name, value_name, odd_value, handicap, bookmaker_name }
  const oddsContainer = data.odds || (data.fixture && data.fixture.odds);
  if (oddsContainer && typeof oddsContainer === 'object') {
    // — 1X2 / Match Winner —
    const mwArr = oddsContainer.match_winner || oddsContainer['1x2'] || oddsContainer.x12;
    if (Array.isArray(mwArr) && mwArr.length) {
      // Group by bookmaker, pick "most common bookmaker" (Bet365 → Marathonbet → Pinnacle → first)
      const preferred = ['Bet365', 'Marathonbet', 'Pinnacle', '1xBet', 'William Hill'];
      let bmName = null;
      for (const p of preferred) {
        if (mwArr.find(o => (o.bookmaker_name || '').toLowerCase().includes(p.toLowerCase()))) {
          bmName = p; break;
        }
      }
      if (!bmName) bmName = mwArr[0].bookmaker_name;
      const sameBm = mwArr.filter(o => (o.bookmaker_name || '') === bmName || (bmName && (o.bookmaker_name || '').toLowerCase().includes(bmName.toLowerCase())));
      const pool = sameBm.length >= 3 ? sameBm : mwArr;
      const home = pool.find(o => /^home$|^1$/i.test(o.value_name || ''));
      const draw = pool.find(o => /^draw$|^x$/i.test(o.value_name || ''));
      const away = pool.find(o => /^away$|^2$/i.test(o.value_name || ''));
      if (home && draw && away) {
        out.x12 = {
          home: parseFloat(home.odd_value),
          draw: parseFloat(draw.odd_value),
          away: parseFloat(away.odd_value),
          bookmaker: home.bookmaker_name
        };
      }
    }

    // ═══ HELPER: pick line có odds CÂN NHẤT (=line MAIN pre-match) ═══
    // API thường trả nhiều ALT lines (-0.5, -1.5, -2.5, -3...), nhưng line MAIN
    // là line mà 2 bên ngang nhau (home ~1.85-1.95 vs away ~1.85-1.95).
    // Các line cực đoan (home 1.02 vs away 8.00) là ALTERNATIVE markets.
    function _pickBalanced(byLine, getH, getA) {
      let best = null, bestScore = Infinity, bestLineVal = 0;
      Object.keys(byLine).forEach(lineKey => {
        const items = byLine[lineKey];
        const sideA = items.find(getH);  // home OR over
        const sideB = items.find(getA);  // away OR under
        if (!sideA || !sideB) return;
        const hOdds = parseFloat(sideA.odd_value);
        const aOdds = parseFloat(sideB.odd_value);
        if (!hOdds || !aOdds || hOdds <= 1 || aOdds <= 1) return;
        // Score = balance + extreme penalty
        // - balance: |h-a| càng nhỏ = càng cân (line MAIN ~ 0.05-0.30)
        // - extreme penalty: phạt odds < 1.30 hoặc > 4.00 (alt lines)
        const balance = Math.abs(hOdds - aOdds);
        const extremePenalty =
          (hOdds < 1.30 ? 10 : 0) + (aOdds < 1.30 ? 10 : 0) +
          (hOdds < 1.50 ? 3  : 0) + (aOdds < 1.50 ? 3  : 0) +
          (hOdds > 4.00 ? 5  : 0) + (aOdds > 4.00 ? 5  : 0);
        const score = balance + extremePenalty;
        if (score < bestScore) {
          bestScore = score;
          best = { home: sideA, away: sideB, over: sideA, under: sideB, count: items.length };
          const lineRaw = sideA.handicap || sideA.total;
          bestLineVal = lineRaw != null ? parseFloat(lineRaw) : parseFloat(lineKey);
          if (isNaN(bestLineVal)) bestLineVal = 0;
        }
      });
      return best ? { pair: best, line: bestLineVal } : null;
    }

    // — Asian Handicap (pick BALANCED line) —
    const ahArr = oddsContainer.asian_handicap || oddsContainer.handicap || oddsContainer.asian;
    if (Array.isArray(ahArr) && ahArr.length) {
      const byLine = {};
      ahArr.forEach(o => {
        const lineRaw = o.handicap;
        const lineKey = lineRaw == null ? 'null' : String(lineRaw);
        if (!byLine[lineKey]) byLine[lineKey] = [];
        byLine[lineKey].push(o);
      });
      const picked = _pickBalanced(
        byLine,
        o => /home/i.test(o.value_name || ''),
        o => /away/i.test(o.value_name || '')
      );
      if (picked) {
        out.ah = {
          line: picked.line,
          homeOdds: parseFloat(picked.pair.home.odd_value),
          awayOdds: parseFloat(picked.pair.away.odd_value),
          bookmaker: picked.pair.home.bookmaker_name
        };
      }
    }

    // — Over/Under (Tài/Xỉu) — pick BALANCED line —
    const ouArr = oddsContainer.goals_over_under || oddsContainer.over_under || oddsContainer.totals || oddsContainer.goals;
    if (Array.isArray(ouArr) && ouArr.length) {
      const byLine = {};
      ouArr.forEach(o => {
        const lineRaw = o.handicap || o.total;
        const lineKey = lineRaw == null ? 'null' : String(lineRaw);
        if (!byLine[lineKey]) byLine[lineKey] = [];
        byLine[lineKey].push(o);
      });
      const picked = _pickBalanced(
        byLine,
        o => /over|tai|t[àa]i/i.test(o.value_name || ''),
        o => /under|xiu|x[ỉi]u/i.test(o.value_name || '')
      );
      if (picked) {
        out.ou = {
          line: picked.line,
          taiOdds: parseFloat(picked.pair.over.odd_value),
          xiuOdds: parseFloat(picked.pair.under.odd_value),
          bookmaker: picked.pair.over.bookmaker_name
        };
      }
    }

    // Nếu đã có ít nhất 1 market → return luôn (không chạy fallback formats)
    if (out.ah || out.ou || out.x12) return out;
  }
...
async function getOdds(fixtureId, opts) {
  ...
  // STRATEGY 1: API endpoint /api/p/fixtures/{id}/detail (CONFIRMED working)
  const raw = await fetchRawOdds(fixtureId);
  if (raw && raw.raw) {
    parsed = parseOdds(raw.raw);
    if (parsed && (parsed.ah || parsed.ou || parsed.x12)) {
      parsed._source = raw.source;
      _cache.set(fixtureId, { data: parsed, expires: now + CACHE_TTL_MS });
      return parsed;
    }
  }

  // STRATEGY 2: Fallback scrape HTML từ thethaoviet.vip
  if (opts.home && opts.away) {
    const scraped = await scrapeOdds(opts.home, opts.away, fixtureId);
    if (scraped && (scraped.ah || scraped.ou || scraped.x12)) {
      _cache.set(fixtureId, { data: scraped, expires: now + CACHE_TTL_MS });
      return scraped;
    }
  }
...
module.exports = { getOdds, debugRaw, fetchRawOdds, parseOdds, scrapeOdds };
