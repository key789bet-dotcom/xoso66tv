...
function groupOdds(rows) {
  ...
  for (const r of rows || []) {
    ...
    if (betId === 4 || betName.includes('handicap') || betName.includes('asian')) {
      if (handicap === '') continue;  // AH BẮT BUỘC có handicap
      const key = bk + '|' + handicap;
      const lineNum = parseFloat(handicap);
      if (isNaN(lineNum)) continue;
      if (!out.asian_handicap[key]) out.asian_handicap[key] = { bookmaker: bk, handicap, line: lineNum };
      ...
      const lineNum = parseFloat(handicap);
      if (isNaN(lineNum)) continue;
      if (!out.over_under[key]) out.over_under[key] = { bookmaker: bk, total: handicap, line: lineNum };
...
async function getOdds(fixtureId, opts) {
  ...
  // AH: pick main line (line=0 hợp lệ → không dùng `|| 0`)
  const ahMain = pickMainLine(grouped.asian_handicap);
  if (ahMain) {
    out.ah = {
      line: (typeof ahMain.line === 'number' && !isNaN(ahMain.line)) ? ahMain.line : 0,
  ...
  // OU: pick main line (key over/under thay vì home/away)
  const ouMain = pickMainLine(grouped.over_under, 'over', 'under');
  if (ouMain) {
    out.ou = {
      line: (typeof ouMain.line === 'number' && !isNaN(ouMain.line)) ? ouMain.line : 2.5,
      taiOdds: ouMain.over,
