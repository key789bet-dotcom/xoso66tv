...
function parseOdds(raw) {
  ...
  if (oddsContainer && typeof oddsContainer === 'object') {
    ...
    if (Array.isArray(ahArr) && ahArr.length) {
      // Group by handicap line, pick most balanced (line gần 0 hoặc line phổ biến nhất)
      const byLine = {};
      ahArr.forEach(o => {
      ...
      // Pick line có nhiều bookmaker nhất + có cả Home và Away
      let bestLine = null, bestPair = null;
      Object.keys(byLine).forEach(lineKey => {
        const items = byLine[lineKey];
        const home = items.find(o => /home/i.test(o.value_name || ''));
        const away = items.find(o => /away/i.test(o.value_name || ''));
        if (home && away) {
          if (!bestPair || items.length > bestPair.count) {
            bestLine = parseFloat(home.handicap) || 0;
            bestPair = { home, away, count: items.length };
          }
        }
      });
      if (bestPair) {
        out.ah = {
          line: bestLine,
          homeOdds: parseFloat(bestPair.home.odd_value),
          awayOdds: parseFloat(bestPair.away.odd_value),
          bookmaker: bestPair.home.bookmaker_name
        };
      }
    }
    ...
    if (Array.isArray(ouArr) && ouArr.length) {
      ...
      Object.keys(byLine).forEach(lineKey => {
        const items = byLine[lineKey];
        const over = items.find(o => /over|tai|t[àa]i/i.test(o.value_name || ''));
        const under = items.find(o => /under|xiu|x[ỉi]u/i.test(o.value_name || ''));
        if (over && under) {
          if (!bestPair || items.length > bestPair.count) {
            bestLine = parseFloat(over.handicap || over.total || lineKey) || 2.5;
            bestPair = { over, under, count: items.length };
          }
        }
      });
      if (bestPair) {
        out.ou = {
          line: bestLine,
          taiOdds: parseFloat(bestPair.over.odd_value),
          xiuOdds: parseFloat(bestPair.under.odd_value),
          bookmaker: bestPair.over.bookmaker_name
        };
      }
    }
