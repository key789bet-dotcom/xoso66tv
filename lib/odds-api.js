  ...
  if (oddsContainer && typeof oddsContainer === 'object') {
    ...
    function _pickBalanced(byLine, getH, getA) {
      ...
      Object.keys(byLine).forEach(lineKey => {
        ...
        const home = items.find(getH);
        const away = items.find(getA);
        if (!home || !away) return;
        const hOdds = parseFloat(home.odd_value);
        const aOdds = parseFloat(away.odd_value);
        ...
        if (score < bestScore) {
          bestScore = score;
          best = { home, away, count: items.length };
          const lineRaw = home.handicap || home.total;
