 ...
 * MAIN line = line có nhiều bookmaker nhất với cùng giá trị handicap
 * (consensus across bookmakers = main pre-match line)
 */
function pickMainLine(items, kHome, kAway) {
  kHome = kHome || 'home';
  kAway = kAway || 'away';
  if (!items || !items.length) return null;
  // Group by handicap line, count bookmakers
  const byLine = {};
  items.forEach(it => {
  ...
  // Pick line có NHIỀU BOOKMAKER NHẤT (consensus = main)
  ...
  if (!best) return null;
  // Trong group đó, ưu tiên bookmaker uy tín (Bet365, Pinnacle, Marathonbet)
  const PREF = ['Bet365', 'Pinnacle', 'Marathonbet', 'William Hill', '1xBet', 'Betano'];
  let chosen = null;
  for (const p of PREF) {
    chosen = best.find(b => (b.bookmaker || '').toLowerCase().includes(p.toLowerCase()));
    if (chosen) break;
  }
  if (!chosen) chosen = best[0];
  return chosen;
}
