...
async function login(username, password, res, options) {
  ...
  if (username === ADMIN_USER) {
    ...
    // 🚀 Phase 2.4.A: dùng API db.users.findByUsername (relational, scale 100k)
    //    Fallback db.load().users.find() nếu adapter cũ
    let user = null;
    if (db.users && typeof db.users.findByUsername === 'function') {
      user = await db.users.findByUsername(username);
    } else {
      const data = db.load();
      user = data.users && data.users.find(u =>
        (u.username || '').toLowerCase() === username.toLowerCase()
      );
    }
    if (!user) return { ok:false, error:'Tài khoản không tồn tại' };
    if (user.banned || user.status === 'banned') return { ok:false, error:'Tài khoản đã bị khóa' };
    ...
    // Update last_login
    try {
      if (db.users && typeof db.users.update === 'function') {
        await db.users.update(user.id, {
          lastLoginAt: new Date(),
          lastLoginIp: (options.ip || '').slice(0, 45)
        });
      }
