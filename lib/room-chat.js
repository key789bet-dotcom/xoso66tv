...
// 🎯 Mục 10: Event listeners để socket.io push tin nhắn realtime
const _listeners = []; // [{ roomId, fn }]
function _emit(roomId, msg) {
  for (let i = 0; i < _listeners.length; i++) {
    if (_listeners[i].roomId === roomId || _listeners[i].roomId === '*') {
      try { _listeners[i].fn(roomId, msg); } catch (_) {}
    }
  }
}
function onMessage(roomId, fn) {
  _listeners.push({ roomId: roomId || '*', fn });
  return function unsubscribe() {
    const idx = _listeners.findIndex(function(l){ return l.fn === fn; });
...
function addMessage(roomId, msg){
  ...
  // 🚀 Mục 10: Emit cho socket.io subscribers
  _emit(roomId, msg);
  return msg;
}
...
module.exports = {
  addMessage, getMessages, getRoom, stats, onMessage,
  // Moderation
