...
function settleMatch(matchId, actualHome, actualAway) {
  ...
  unsettled.forEach(p => {
    ...
    if (p.pickOU) {
      ...
      if (correctOU) correctCount++;
      if (actualOU === 'push') correctOU = null;  // push không tính
    }
    if (p.pickAH) {
      const actualAH = evalAH(actualHome, actualAway, p.ahLine);
      correctAH = actualAH === p.pickAH;
      if (correctAH) correctCount++;
      if (actualAH === 'push') correctAH = null;
    }

    if (correctCount === 2) { points = 100; reward = 100; }
    else if (correctCount === 1) { points = 30; reward = 30; }
